...
function getStressLabel(val: number): string {
  if (val <= 2) return "Calm"; if (val <= 4) return "Mild"; if (val <= 6) return "Moderate"; if (val <= 8) return "High"; return "Extreme";
}
...
function getStressColor(val: number): string {
  if (val <= 2) return "#34d399"; if (val <= 4) return "#38bdf8"; if (val <= 6) return "#fbbf24"; if (val <= 8) return "#fb7185"; return "#ef4444";
}
      ...
      <div className="glass-card p-4 glow-rose">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-accent-rose/20 to-orange-400/20 flex items-center justify-center backdrop-blur-sm">
        ...
        <div className="relative">
          <input type="range" min={1} max={10} step={1} value={value} onChange={(e) => onChange(Number(e.target.value))}
            ...
            style={{ background: `linear-gradient(to right, #34d399 0%, ${color} ${pct}%, var(--slider-track) ${pct}%)` }} />
          <div className="absolute top-0 left-0 h-1.5 rounded-full pointer-events-none transition-all duration-200"
            style={{ width: `${pct}%`, boxShadow: `0 0 12px ${color}50, 0 0 4px ${color}30`, minWidth: '2px' }} />
        </div>
        <div className="flex justify-between mt-3 items-center">
          <span className="text-[10px] text-accent-green font-medium">Calm</span>
          <motion.span key={value} initial={{ opacity: 0, scale: 0.8, y: 4 }} animate={{ opacity: 1, scale: 1, y: 0 }}
            className="text-xs font-semibold px-3 py-1 rounded-full border backdrop-blur-sm"
            style={{ color, backgroundColor: `${color}12`, borderColor: `${color}25`, boxShadow: `0 0 16px ${color}15` }}>
