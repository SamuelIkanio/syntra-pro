...
interface Props { energy: number; mood: number; onEnergyChange: (v: number) => void; onMoodChange: (v: number) => void; }
...
function SliderCard({ icon: Icon, label, value, emoji, gradientFrom, gradientTo, thumbColor, glowClass, onChange, delay }: {
  icon: React.ElementType; label: string; value: number; emoji: string; gradientFrom: string; gradientTo: string;
  thumbColor: string; glowClass: string; onChange: (v: number) => void; delay: number;
}) {
    ...
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay }} className={`glass-card p-4 ${glowClass}`}>
      ...
      <div className="relative">
        ...
        <input type="range" min={1} max={10} step={1} value={value} onChange={(e) => onChange(Number(e.target.value))}
          className="w-full h-1.5 rounded-full cursor-pointer relative z-10"
