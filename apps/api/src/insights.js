  ...
  return new Promise((resolve, reject) => {
    ...
    if (!fs.existsSync(mainPy)) {
      ...
      return resolve({
        ...
        insights: [],
        message: "Insight engine not available. Install the Python engine in packages/insight-engine.",
      });
    }
    ...
    execFile(PYTHON, [mainPy, tmpFile, "--json"], {
      ...
      if (err) {
        console.error("Insight engine stderr:", stderr);
        return reject(new Error(`Insight engine failed: ${err.message}. stderr: ${stderr}`));
      }
      ...
      try {
        ...
        reject(new Error(`Failed to parse engine output: ${parseErr.message}. stdout: ${stdout.slice(0, 500)}`));
      }
    });
  });
}
