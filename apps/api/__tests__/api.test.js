...
describe("POST /api/auth/register", () => {
  it("registers a new user", async () => {
    const res = await request(app)
      ...
      .send({ email: "test@triune.com", password: "securePass123", name: "Test User" });
  ...
  it("rejects duplicate email", async () => {
    const res = await request(app)
      .post("/api/auth/register")
      .send({ email: "test@triune.com", password: "anotherPass1", name: "Dup User" });
  ...
  it("rejects invalid email", async () => {
    const res = await request(app)
      .post("/api/auth/register")
      .send({ email: "not-an-email", password: "securePass123", name: "Bad" });
...
describe("POST /api/auth/login", () => {
  it("logs in with valid credentials", async () => {
    const res = await request(app)
      .post("/api/auth/login")
      .send({ email: "test@triune.com", password: "securePass123" });
  ...
  it("rejects wrong password", async () => {
    const res = await request(app)
      .post("/api/auth/login")
      .send({ email: "test@triune.com", password: "wrongpassword" });
...
describe("POST /api/logs (authenticated)", () => {
  it("rejects unauthenticated request", async () => {
    const res = await request(app)
      .post("/api/logs")
      .send({ date: "2026-04-20", energy: 7, mood: 8, stress: 3 });
    expect(res.status).toBe(401);
  });
  ...
  it("creates a daily log", async () => {
    const res = await request(app)
      ...
      .send({
        ...
        meals: [
          { type: "Breakfast", description: "Oatmeal with fruit", healthScore: 4 },
          { type: "Lunch", description: "Grilled chicken salad", healthScore: 5 },
        ],
      });
  ...
  it("validates date format", async () => {
    const res = await request(app)
      .post("/api/logs")
      .set("Authorization", `Bearer ${authToken}`)
      .send({ date: "20-04-2026", energy: 5, mood: 5, stress: 5 });
    expect(res.status).toBe(400);
  });
  ...
  it("validates energy range", async () => {
    const res = await request(app)
      .post("/api/logs")
      .set("Authorization", `Bearer ${authToken}`)
      .send({ date: "2026-04-21", energy: 15, mood: 5, stress: 5 });
    expect(res.status).toBe(400);
  });
  ...
  it("validates mood range", async () => {
    const res = await request(app)
      .post("/api/logs")
      .set("Authorization", `Bearer ${authToken}`)
      .send({ date: "2026-04-21", energy: 5, mood: 0, stress: 5 });
    expect(res.status).toBe(400);
  });
  ...
  it("upserts on same date", async () => {
    await request(app)
      .post("/api/logs")
      .set("Authorization", `Bearer ${authToken}`)
      .send({ date: "2026-04-25", energy: 5, mood: 5, stress: 5, symptoms: ["fatigue"], meals: [] });

    const res = await request(app)
      .post("/api/logs")
      .set("Authorization", `Bearer ${authToken}`)
      .send({ date: "2026-04-25", energy: 9, mood: 9, stress: 1, symptoms: ["headache"], meals: [] });

    expect(res.status).toBe(201);

    const getRes = await request(app)
      .get("/api/logs/2026-04-25")
      .set("Authorization", `Bearer ${authToken}`);
...
describe("GET /api/logs (authenticated)", () => {
  ...
  it("supports date filtering", async () => {
    await request(app)
      .post("/api/logs")
      .set("Authorization", `Bearer ${authToken}`)
      .send({ date: "2026-04-18", energy: 6, mood: 6, stress: 4, symptoms: [], meals: [] });

    const res = await request(app)
      .get("/api/logs?from=2026-04-18&to=2026-04-20")
      .set("Authorization", `Bearer ${authToken}`);
...
describe("DELETE /api/logs/:date (authenticated)", () => {
  it("deletes a log", async () => {
    await request(app)
      .post("/api/logs")
      .set("Authorization", `Bearer ${authToken}`)
      .send({ date: "2026-04-10", energy: 5, mood: 5, stress: 5, symptoms: [], meals: [] });

    const res = await request(app)
      .delete("/api/logs/2026-04-10")
      .set("Authorization", `Bearer ${authToken}`);
    expect(res.status).toBe(200);
    expect(res.body.message).toBe("Log deleted successfully");

    const getRes = await request(app)
      .get("/api/logs/2026-04-10")
      .set("Authorization", `Bearer ${authToken}`);
...
describe("User isolation", () => {
  ...
  beforeAll(async () => {
    const res = await request(app)
      .post("/api/auth/register")
      .send({ email: "user2@triune.com", password: "securePass456", name: "User Two" });
    user2Token = res.body.token;

    await request(app)
      .post("/api/logs")
      .set("Authorization", `Bearer ${user2Token}`)
      .send({ date: "2026-04-20", energy: 3, mood: 3, stress: 8, symptoms: [], meals: [] });
  });
  ...
  it("user1 cannot see user2 data on same date", async () => {
    const res = await request(app)
      .get("/api/logs/2026-04-20")
      .set("Authorization", `Bearer ${authToken}`);
    expect(res.body.energy).toBe(7); // user1's data
  });
});
